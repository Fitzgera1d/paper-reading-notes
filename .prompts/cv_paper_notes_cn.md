...
3. **区分信息来源**
   - 默认情况下，正文内容均视为来自论文，不必对来自论文的内容逐句标注 `[Paper]`。
   - 只有在以下情况才显式标注来源：
     - `[Code]`：代码中可直接确认，且该信息补全了论文未充分说明的细节
     - `[Inference]`：根据论文和代码做出的合理推断
     - `未确认`：论文与代码都无法确认的关键细节
   - 如果一整节主要依据论文正文与附录，可在节首统一说明“本节除特别说明外均依据论文正文与附录”，不要在句尾反复添加 `[Paper]`。
...
如果论文没有写清，而代码能补全，请用 `[Code]` 标注；如果只能合理推断，请用 `[Inference]` 标注；如果论文和代码都无法确认，请直接写“未确认”。
...
- 不要对来自论文的内容逐句添加 `[Paper]` 标注；论文内容默认不标，只有 `[Code]`、`[Inference]` 和“未确认”需要显式标出
