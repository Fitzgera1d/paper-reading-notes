* # 论文标题: SAM 3D Body: Robust Full-Body Human Mesh Recovery - [arXiv] 2025
  ...
  *   **详细网络架构与数据流**:
      ...
      3.  **Token 组装 (Queries)**:
          ...
          *   $T_{pose}$ (1 token): 初始化为零姿态的可学习向量，用于回归核心 MHR 参数。
          *   $T_{kpts}$ (70 tokens): 对应 MHR 的 70 个关键点，用于显式建模关节位置。
          
          > ### 实际的 Token 队列结构
          >
          > 在代码中，解码器的输入 `token_embeddings` 是通过 `torch.cat` 逐步拼接而成的。实际顺序如下：
          > $$
          ...
          >      - 它不像 $T_{pose}$ 那样演化。它被放入 `token_augment` 变量中。在 Transformer 的每一层计算 Attention 之前，它会被**重新加回去** (Residual Connection or Re-injection)。
